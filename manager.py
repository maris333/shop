...
class Manager:
    def __init__(self, quantity_in_stock: Dict[Order, int]):
        self.orders_in_stock = quantity_in_stock
    ...
    def check_if_order_is_in_stock(self, order: Order, amount: int, id: int):
        if self.get_order_in_stock_by_id(id) is not None:
            order_in_stock = self.get_order_in_stock_by_id(id)
    ...
    def add_order(self, order: Order, amount: int, id: int):
        if self.check_if_order_is_in_stock(order, amount, id):
            order_in_stock = self.get_order_in_stock_by_id(id)
    ...
    def delete_order(self, order: Order, amount: int, id: int):
        if order in self.orders:
            order_in_stock = self.get_order_in_stock_by_id(id)
